...
class MotelController extends Controller
{
	public function SearchMotelAjax(Request $request){
		$getmotel = Motelroom::where([
			['address', 'like', '%'.$request->id_ditrict.'%'],
	...
	public function getMotelByCategoryId($id){
		...
		return view('home.category',['listmotel'=>$getmotel,'categories'=>$Categories,'notification'=>$notification]);
	}
	...
	public function userReview($id, Request $request)
	{
		if (Auth::user()) {
			...
			$review->save();
			$getmotel = Motelroom::find($id);
			return redirect('phongtro/'.$getmotel->slug)->with('thongbaoreview','Bình luận của bạn đã được gửi đi, vui lòng chờ duyệt');
		}
		else{
			$getmotel = Motelroom::find($id);
			return redirect('phongtro/'.$getmotel->slug)->with('thongbaoreview','Bạn vui lòng đăng nhập để có thể bình luận');
		}
	}
}
